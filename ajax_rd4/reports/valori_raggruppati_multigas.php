...
$page_title = "Report - valori raggruppati MULTIGAS";
$page_id = "report_valori_raggruppati_multigas";
...
$html.=' <h3>Raggruppamento Valori MultiGAS</h3>
...
foreach($O->lista_gas_partecipanti() as $rowG){
    ...
    $html.='<tr>
                <th colspan=4 style="text-align:left">'.$G->descrizione_gas.'</th>
            </tr>';
    ...
    foreach($rows AS $row){
        ...
        $sql = "SELECT qta_arr, prz_dett_arr FROM retegas_dettaglio_ordini inner join maaking_users U on U.userid=id_utenti WHERE id_ordine=:id_ordine AND U.id_gas=:id_gas AND art_codice=:art_codice ";
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':id_ordine', $O->id_ordini, PDO::PARAM_INT);
        $stmt->bindParam(':id_gas', $G->id_gas, PDO::PARAM_INT);
...
<script type="text/javascript">
    ...
    var pagefunction = function(){
        ...
        $('.show_pdf').click(function(){
            ...
            open('POST', '<?php echo APP_URL; ?>/ajax_rd4/reports/valori_raggruppati_multigas.php', {id:id, o:'pdf', dummy:<?php echo rand(1000,9999); ?> }, '_blank');
            return false;
        });
        ...
        $('#aggiorna_report').click(function(){
            ...
            location.replace('<?php echo APP_URL; ?>/#ajax_rd4/reports/valori_raggruppati_multigas.php?id='+id+'&ro='+ro+'&rg='+rg);
            return false;
        });
