...
if(!_USER_GAS_VISIONE_DATI_AMICI){
    echo rd4_go_back("Il tuo gas non lo permette...");
    die();
}
...
$page_title = "Report - articoli ordinati distribuzione amici";
$page_id = "report_articoli_distribuzione_amici";
...
foreach($rows AS $rowU){
    ...
    //CONTEGGIO AMICI
    $n_amici = count($O->lista_amici_partecipanti($rowU["userid"]));



    $sql = "SELECT IFNULL(A.nome,:fullname) as nome , IFNULL(A.telefono,'') as telefono , IFNULL(A.id_amici, 0) as id_amici, id_user
                FROM retegas_distribuzione_spesa D
                LEFT JOIN retegas_amici A ON A.id_amici = D.id_amico
                WHERE D.id_ordine=:id_ordine
                AND D.id_user='".$rowU["userid"]."'
                ".$where_amico."
                GROUP BY A.id_amici
                ".$order_amico.";";
    $stmt = $db->prepare($sql);
    $stmt->bindParam(':id_ordine', $O->id_ordini, PDO::PARAM_INT);
    $stmt->bindParam(':fullname', $rowU["fullname"]);
    $stmt->execute();
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    foreach($rows AS $rowUU){
          $html_Rett ="";
          ...
          $html_ut .='<tr class="utente">';
            if($rowUU["nome"]==$rowU["fullname"]){
                $html_ut .='<td colspan=3><strong>'.$rowUU["nome"].'</strong></td>';
            }else{
                $html_ut .='<td colspan=3><strong>'.$rowUU["nome"].'</strong>, amico di '.$rowU["fullname"].'</td>';
            }
            $html_ut .='<td class="text-center">Qta ORD.</td>';
          ...
          $sql = "SELECT D.prz_dett_arr, D.prz_dett,D.art_codice, D.art_desc, D.art_um, A.qta_ord, A.qta_arr, D.id_articoli
                    ...
                    WHERE D.id_ordine=:id_ordine
                    AND D.id_utenti='".$rowU["userid"]."'
                    AND A.id_amico=:id_amico
                    AND LEFT(D.art_codice , 2)<>'@@'
                    AND LEFT(D.art_codice , 2)<>'##'";
          $stmt = $db->prepare($sql);
          $stmt->bindParam(':id_ordine', $O->id_ordini, PDO::PARAM_INT);
          $stmt->bindParam(':id_amico', $rowUU["id_amici"], PDO::PARAM_INT);
          $stmt->execute();
      ...
      foreach($rowsA AS $rowA){
          ...
          $qta_ord = $rowA["qta_ord"];
          $prz_dett_arr = $rowA["prz_dett_arr"];
          $prz_dett_ord = $rowA["prz_dett"];
          $tot_riga = $rowA["prz_dett"]*$rowA["qta_ord"];
          ...
          $html_A ='<tr>';
            ...
            $html_A .='<td class="text-center">'.rd4_nf($qta_ord,$decimals).'</td>';
            $html_A .='<td class="text-right">'.rd4_nf($prz_dett_ord,$decimals).'</td>';
            $html_A .='<td class="text-right">'.rd4_nf($tot_riga,$decimals).'</td>';
            $html_A .='<td></td>';
            $html_A .='<td></td>';
            $html_A .='<td class="text-right"></td>';
            ...
            $stmtN->bindParam(':id_user', $rowUU["id_user"], PDO::PARAM_INT);
            $stmtN->execute();
            $rowN = $stmtN->fetch();
          ...
          $html .='<tr class="subtotale">';
            $html .='<td colspan=5 class="text-right">Totale di '.$rowUU["nome"].':</td>';
...
<script type="text/javascript">
    ...
    var pagefunction = function(){
        ...
        $('.show_pdf').click(function(){
            ...
            open('POST', '<?php echo APP_URL; ?>/ajax_rd4/reports/articoli_distribuzione.php', {id:id, o:'pdf', dummy:<?php echo rand(1000,9999); ?>, a: a, re: re, s:s}, '_blank');
            return false;
        });
        $('#aggiorna_report').click(function(){
            ...
            re = $('input[name="radio-rett"]:checked').val();
            console.log('S='+s);
            location.replace('<?php echo APP_URL; ?>/#ajax_rd4/reports/articoli_distribuzione.php?id='+id+'&a='+a+'&s='+s+'&re='+re);
            return false;
        });
