...
$page_title = "Listini del mio DES";
$page_id = "des_listini";



$stmt = $db->prepare("SELECT * FROM retegas_listini inner join maaking_users on userid=id_utenti 
                        inner join retegas_gas G on G.id_gas=maaking_users.id_gas
                            WHERE id_des='"._USER_ID_DES."' AND is_deleted=0 ORDER BY data_valido DESC");
    ...
    foreach($rows as $row){
        ...
        if(($row["is_privato"]==1) AND ($utente["id_gas"]<>_USER_ID_GAS)){
            $n_privato ++;
            $li.= '<li class="list-group-item privato" style="display:none;" >
            ...
            $li.= '<li class="list-group-item '.$class_scaduto.'" '.$style_scaduto.' >
                        <span>
                            '.$utente["fullname"].' ('.$utente["descrizione_gas"].'):
                            '.$icona_scade.'
    ...
    $title_navbar='Listini del mio DES';
    if(_USER_PERMISSIONS & perm::puo_creare_listini){
        ...
        <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <form class="smart-form">
                ...
                <div>
                    ...
                    <button class="pull-right btn btn-xs btn-default" id="mostra_privati">Mostra anche quelli privati <strong>(<?php echo $n_privato;?>)</strong></button>
                </div>
...
<script type="text/javascript">
    ...
    var pagefunction = function(){
        ...
        $(document).off('click','#mostra_privati');
        $(document).on('click','#mostra_privati',function(e){
