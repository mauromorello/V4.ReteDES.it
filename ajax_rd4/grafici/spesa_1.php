...
$page_title = "Spesa del GAS, DES e reteDES";
$page_id ="spesa_1";
...
$SQL = "select
        ...
        round(sum(qta_arr*prz_dett_arr),2) as euro
...
foreach($rows as $row){
    $euro += $row["euro"];
    $RdesData .="[Date.UTC(".$row["jdate"]."),".$euro."],";    
}
$SQL = "select
        date_format(timestamp_ord, '%Y,%m,%d') as ndate,
        CONCAT_WS(',',CAST(extract(year from timestamp_ord) AS CHAR)
...
foreach($rows as $row){
    $euroG += $row["euro"];
    $gasData .="[Date.UTC(".$row["jdate"]."),".$euroG."],";    
}
$SQL = "select
        date_format(timestamp_ord, '%Y,%m,%d') as ndate,
        CONCAT_WS(',',CAST(extract(year from timestamp_ord) AS CHAR)
...
foreach($rows as $row){
    $euroD += $row["euro"];
    $desData .="[Date.UTC(".$row["jdate"]."),".$euroD."],";    
}
