...
function get_user_iban($userid){
    global $db;
    $stmt = $db->prepare("SELECT valore_text from retegas_options WHERE id_user='".$userid."' AND chiave='_USER_IBAN'");
    $stmt->execute();
    $row = $stmt->fetch();
    return $row["valore_text"];
}
function get_user_mandate_id($userid){
    global $db;
    $stmt = $db->prepare("SELECT valore_text from retegas_options WHERE id_user='".$userid."' AND chiave='_USER_MANDATE_ID'");
    $stmt->execute();
    $row = $stmt->fetch();
    return $row["valore_text"];
}
function get_user_mandate_date($userid){
    global $db;
    $stmt = $db->prepare("SELECT valore_data from retegas_options WHERE id_user='".$userid."' AND chiave='_USER_MANDATE_DATE'");
    $stmt->execute();
    $row = $stmt->fetch();
    return substr($row["valore_data"],0,10);
    
}

//-------------------------------PAGE
$page_title = "Report - riepilogo cassa";
$page_id = "report_riepilogo_cassa";
...
//------------------------------DO SEPA
$do_sepa=CAST_TO_INT($_GET["do_sepa"],0);
if($do_sepa==1){
    require_once('../../lib_rd4/SEPA/SEPASDD.php');
    $data_sepa=CAST_TO_STRING($_GET["data_sepa"]);
    $titolo_sepa=CAST_TO_STRING($_GET["titolo_sepa"]);
    $tipo_sepa=CAST_TO_STRING($_GET["tipo_sepa"]);
    $versione_sepa=CAST_TO_INT($_GET["versione_sepa"],2,3);
    
    if($titolo_sepa==''){
        $titolo_sepa= clean(_USER_GAS_NOME);
    }
    if($data_sepa==''){
        $SEPA_error .= 'MANCA la data di esecuzione.'.'<br>';
        die($SEPA_error);
    }
    if($tipo_sepa==''){
        $tipo_sepa=='OOFF';
...
$id_utente=CAST_TO_INT($_GET["id_utente"],0);
if($id_utente==0){
...
if($do_sepa==1){
    ...
    if($C->_GAS_ID_CREDITORE==''){
        die("Manca ID Creditore GAS");    
    }
    try{
        $config = array(    "name" => _USER_GAS_NOME,
                            "IBAN" => $C->_GAS_IBAN,
                            "batch" => True,
                            "creditor_id" => $C->_GAS_ID_CREDITORE,
                            "currency" => "EUR",
                            "version" => $versione_sepa
                            );
...
$html.='<table id="table_exp" class="rd4" style="margin-left: auto; margin-right: auto">
            <thead >
                <tr class="intestazione" >
                    ...
                    <th style="width:5%;" class="text-right">Credito</th>
                    <th style="width:5%;" class="text-right">Debito</th>
                    <th style="width:5%;" class="text-right">Saldo</th>
                </tr>
            </thead>
            <tbody>';
...
$sql = "SELECT R.id_cassa_utenti AS id, U.fullname, U.userid, U.email, DATE_FORMAT(R.data_movimento,'%d/%m/%Y %h:%i') as data, tipo_movimento as tipo, segno, importo, id_ordine, id_ditta, D.descrizione_ditte, U2.fullname as cassiere, DATE_FORMAT(R.data_registrato,'%d/%m/%y %h:%i') as data_registrato, registrato, descrizione_movimento
    ...
    if($old_id<>$new_id){
        if($ucount>0){
            if($nascondi_totali==0){    
                ...
                $html.='<tr class="subtotale">
                            <th colspan=5 style="text-align:left">Totale utente #'.$old_id.' '.$old_fullname.':</th>
            ...
            if($do_sepa==1){
                try{
                    $amount = (round(($totale_credito_utente-$totale_debito_utente),2)*100);
                    if($amount<0){
                        $amount = abs($amount);
                        $is_negative=true;
                    }else{
                    ...
                    $mandate_id= get_user_mandate_id($old_id);
                    $mandate_date = get_user_mandate_date($old_id);
                    $user_iban = get_user_iban($old_id);
                    
                    //echo "AMOUNT:".$amount.' MANDATE ID:'.$mandate_id.' MANDATE DATE '.$mandate_date.' IBAN:'.$user_iban;
                    
                    if($is_negative){
                        if($user_iban<>""){
                            if($mandate_id<>""){
                                if($mandate_date<>""){
                                    $payment = array("name" => clean($old_fullname),
                                                     "IBAN" => $user_iban,
                                                     "amount" => $amount,
                                                     "type" => $tipo_sepa,
                                                     "collection_date" => conv_only_date_to_db($data_sepa),
                                                     "mandate_id" => $mandate_id,
                                                     "mandate_date" => $mandate_date,
                                                     "description" => $titolo_sepa
                                                    );
        ...
        if($nascondi_intestazioni==0){
            ...
            $html.='<tr class="totale">
                        <th colspan=8 style="text-align:left">Utente: '.$row["fullname"].'</th>
                    </tr>';
        }
        ...
        if(($data_da<>"") and $nascondi_pregressi==0){
                ...
                $html.='<tr class="">
                            <td colspan=5 style="text-align:left">Movimenti pregressi alla data '.$data_da.'</th>
...
if($do_sepa==1){
    try{
        ...
        if($SEPA_error==''){
            ...
            header('Content-Disposition: attachment; filename="'.str_replace(' ','_',strtoupper(clean(_USER_GAS_NOME))).'-'.date('Ymdhhs').'.xml"');
            echo($xml);
            die();
        }
    }catch(Exception $e){
        $SEPA_error .= 'Errore salvataggio SEPA:'.$e->getMessage().'<br>';
        //exit;    
    }
}
...
if($SEPA_error<>''){
    ...
    $SEPA_error = '<p class="alert alert-danger margin-top-10"><strong>ATTENZIONE: </strong> non è stato possibile esportare il tracciato SEPA a causa di questo errore:
                    '.$SEPA_error.'
                    </p>';
    die($SEPA_error);                    
}
...
$html.='    </tbody>
            <tfoot>
                ...
                <tr class="totale">
                    <th colspan=5 style="text-align:left">Totali</th>
...
if($_POST["o"]=="pdf"){
  ...
  <head>
    ...
    <title>ReteDES.it :: Riepilogo Cassa</title>
    '.pdf_css().$css.'
  </head>
...
$buttons[]='<button  class="do_sepa btn btn-defalut btn-default"><i class="fa fa-bank"></i><span class="hidden-xs">  SEPA</span></button>';
...
<div class="margin-top-10 well well-lg" id="opzioni_report">
        ...
        <form class="smart-form">
                ...
                <div class="row">
                    ...
                    <section class="col col-3">
                        <label class="input">
                            <input type="text" placeholder="ID utente" id="id_utente" name="id_utente" <?php if($id_utente>0){echo 'value="'.$id_utente.'"';} ?>>
                        </label>
                    </section>
                    <section class="col col-3">
                        <label class="input">
                ...
                <div class="row">
                    <label class="label"><strong>Solo per esportazione SEPA:</strong></label>
                    <section class="col col-5">
                        <label class="input">
                            <input type="text" placeholder="Nome operazione" id="titolo_sepa" name="titolo_sepa" <?php if($titolo_sepa<>''){echo 'value="'.$titolo_sepa.'"';} ?>>
                        </label>
                    </section>
                    <section class="col col-3">
                        <label class="input">
                            <input type="text" placeholder="Data operazione" id="data_sepa" name="data_sepa" data-mask="99/99/9999" <?php if($data_sepa<>''){echo 'value="'.$data_sepa.'"';} ?>>
                        </label>
                    </section>
                    <section class="col col-2">
                        <label class="input">
                            <input type="text" placeholder="Tipo operazione" id="tipo_sepa" name="tipo_sepa" value="OOFF">
                        </label>
                    </section>
                    <section class="col col-2">
                        <label class="input">
                            <input type="text" placeholder="Versione SEPA" id="versione_sepa" name="versione_sepa" value="2">
                        </label>
                    </section>
                </div>                
                <?php echo $SEPA_error; ?>
...
<script type="text/javascript">
    ...
    var pagefunction = function(){
        ...
        $(document).off('click','.do_sepa');
        $(document).on('click', '.do_sepa', function(e){
            $.SmartMessageBox({
                title : "TRACCIATO SEPA",
                content : "Verrà scaricato un file SEPA con gli addebiti corrispondenti ai SALDI di ogni utente.",
                buttons : "[Esci][SCARICA]"
            }, function(ButtonPress, Value) {
                if(ButtonPress=="SCARICA"){
                    ...
                    var tipo_sepa = $('#tipo_sepa').val();
                    if (typeof tipo_sepa === 'undefined' || !tipo_sepa){
                        var SEPAtip = "";
                    }else{
                        var SEPAtip = '&tipo_sepa='+tipo_sepa;    
                    }
                    
                    var data_sepa = $('#data_sepa').val();
                    if (typeof data_sepa === 'undefined' || !data_sepa){
                        var SEPAdat = "";
                    }else{
                        var SEPAdat = '&data_sepa='+data_sepa;    
                    }
                    
                    var titolo_sepa = $('#data_sepa').val();
                    if (typeof titolo_sepa === 'undefined' || !titolo_sepa){
                        var SEPAtit = "";
                    }else{
                        var SEPAtit = '&titolo_sepa='+titolo_sepa;    
                    }
                    
                    var versione_sepa = $('#versione_sepa').val();
                    if (typeof versione_sepa === 'undefined' || !versione_sepa){
                        var SEPAver = "";
                    }else{
                        var SEPAver = '&versione_sepa='+versione_sepa;    
                    }
                    
                    location.replace('<?php echo APP_URL; ?>/ajax_rd4/cassa/riepilogo_cassa.php?'+ido+idu+idd+tpm+nsd+nst+nsi+nsr+nsnr+nsdb+nscr+nspr+dtda+dta+SEPAtip+SEPAdat+SEPAtit+SEPAver+'&s='+s+'&do_sepa=1');
                    return false;                        
                }
            });        
        });
        ...
        $(document).on('click', '.send_mail', function(e){
            $.SmartMessageBox({
                title : "MANDA MAIL",
                content : "Verrà inviata una mail ad ogni utente con la sua situazione cassa come l\'hai filtrata.",
                buttons : "[Esci][INVIA]"
            }, function(ButtonPress, Value) {
                ...
                if(ButtonPress=="INVIA"){
                    ...
                    location.replace('<?php echo APP_URL; ?>/#ajax_rd4/cassa/riepilogo_cassa.php?'+ido+idu+idd+tpm+nsd+nst+nsi+nsr+nsnr+nsdb+nscr+nspr+dtda+dta+'&s='+s+'&send_mail=1');
                    return false;
       ...
       // $('.show_pdf').click(function(){
            ...
            open('POST', '<?php echo APP_URL; ?>/ajax_rd4/cassa/riepilogo_cassa.php', { o:'pdf', 
        ...
        $('#aggiorna_report').click(function(){
            ...
            location.replace('<?php echo APP_URL; ?>/#ajax_rd4/cassa/riepilogo_cassa.php?'+ido+idu+idd+tpm+nsd+nst+nsi+nsr+nsnr+nsdb+nspr+nscr+dtda+dta+'&s='+s);
            return false;
        });
